...
protected:
	...
	double C;					// heat capacity [J/kgK] (enthalpy change with respect to temperature)
	//double CT;					// enthalpy change with respect to pressure
...
public:
	...
	double get_C(double T = 0) { return C; }
	//double get_CT(double T = 0) { return CT; }
	double get_lambda(double T = 0) { return lambda; }
	double get_rho(double p, double T);
	double get_mu(double p, double T);
	double get_K(double p, double T);
