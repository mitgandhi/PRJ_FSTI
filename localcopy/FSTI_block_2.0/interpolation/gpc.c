...
// ------------------------------------------------------------------------- //
// added by Marco to create a polygon without using an input file
// no holes can be defined (I didn't need it)

void gpc_create_polygon(int nc, int *nv, double **x, double	**y, gpc_polygon *p)
{
	int c, v;
	...
	for (c = 0; c < p->num_contours; c++)
  {
    ...
    MALLOC(p->contour[c].vertex, p->contour[c].num_vertices
		...
		for (v = 0; v < p->contour[c].num_vertices; v++)
		{
			p->contour[c].vertex[v].x = x[c][v];
			p->contour[c].vertex[v].y = y[c][v];
		}
  }
}
